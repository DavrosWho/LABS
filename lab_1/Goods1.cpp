...
void Goods::changeQuantity(int aGoodsQuantity) {
	if (aGoodsQuantity < 2147483647 && aGoodsQuantity >= 0) {
		goodsQuantity = aGoodsQuantity;
	}
	else {
		throw exception("The quantity is too large or less than zero");
	}
}

void Goods::changeName(char* aName) {
	if (strlen(aName) <= strlen(goodsName)) {
		for (int i = 0; i < 15; i++) {
			goodsName[i] = aName[i];
		}
	}
	else {
		throw exception("Value out of range of valid memory");
	}
}

void Goods::changeWCost(int aWCost) {
	if (aWCost < 2147483647 && aWCost >= 0 && aWCost <= retailCost) {
		wholesaleCost = aWCost;
	}
	else {
		throw exception("The wholesale cost is too large, less than zero or more than retail cost");
	}
}

void Goods::markup(int aMarkup) {
	if (aMarkup < 2147483647 && aMarkup >= 0 && wholesaleCost + aMarkup < 2147483647) {
		retailCost = wholesaleCost + aMarkup;
	}
	else {
		throw exception("The markup is too large or less than zero");
	}
}

void Goods::markdown(int aMarkdown) {

	if (aMarkdown < 2147483647 && aMarkdown >= 0 && retailCost - aMarkdown >= wholesaleCost) {
		retailCost = retailCost - aMarkdown;
	}
	else {
		throw exception("The markdown is too large, less than zero or more than wholesale cost");
	}
}
